...
from lm_tuio.api import check_server_status, fetch_available_models
...
class LMStudioApp(App):
    ...
    BINDINGS = [("q", "quit", "[quit]"), ("r", "refresh_models", "[refresh list]")]
    ...
    def __init__(self, ip: str, port: int):
        ...
        self.title = (
            f"LM TUIO - LM Studio Dashboard " + f"[{self.target_ip}:{self.target_port}]"
        )
    ...
    async def on_mount(self) -> None:
        ...
        table.add_columns(
            "Model Name", "Architecture", "Size (GB)", "Quantization", "API Key"
        )
    ...
    async def action_refresh_models(self) -> None:
        ...
        if models:
            ...
            for model in models:
                ...
                quant: QuantizationInfo | str = (
                    model.quantization if model.quantization else "None"
                )
                size_gb: str = f"{model.size_bytes / (1024**3):.2f}"

                table.add_row(model.display_name, arch, size_gb, quant, model.key)
