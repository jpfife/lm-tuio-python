...
class ContextPane(Static):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ctx_insts_table: models.ModelInstanceTable = models.ModelInstanceTable(
            id="ctx-insts-table"
        )
    ...
    def _update_base_model_info(self, model: models.ModelInfo) -> None:
        ...
        self.ctx_max_context_val.update(
            f"{models.format_bytes(model.max_context_length)}"
        )
    ...
    def _update_loaded_model_info(self, model: models.ModelInfo) -> None:
        ...
        for mdl in model.loaded_instances:
            ...
            ctx_str = f"{models.format_bytes(mdl.config.context_length)} ({mdl.config.context_length})"

            context_bytes += models.estimate_context_cache_memory(
                model.size_bytes, mdl.config.context_length
            )

            self.ctx_insts_table.add_row(ids_str, ctx_str)

        memory_str: str = f"{models.format_bytes(size_bytes)}"
        kv_mem_str: str = (
            f"KV Q16 - {models.format_bytes(context_bytes + size_bytes)}\n"
        )
        kv_mem_str += (
            f"KV Q8  - {models.format_bytes((context_bytes // 2) + size_bytes)}\n"
        )
        kv_mem_str += (
            f"KV Q4  - {models.format_bytes((context_bytes // 4) + size_bytes)}"
        )

        self.ctx_total_memory_val.update(memory_str)
        self.ctx_kv_memory_val.update(kv_mem_str)

    def update_model_context(self, model: models.ModelInfo | None) -> None:
