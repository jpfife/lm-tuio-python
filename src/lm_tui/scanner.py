...
async def scan_targets(config: AppConfig) -> tuple[list[str] | None, str | None]:
    ...
    if config.is_network:
        for host in network.hosts():
            task: asyncio.Task[str | None] = asyncio.create_task(check_host(str(host), config.port))
            tasks.append(task)
    else:
        single_ip: str = str(network.network_address)
        task: asyncio.Task[str | None] = asyncio.create_task(check_host(single_ip, config.port))
        tasks.append(task)
